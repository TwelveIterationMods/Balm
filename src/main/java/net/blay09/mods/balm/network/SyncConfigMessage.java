...
public class SyncConfigMessage<TData> {
    ...
    public static <TMessage extends SyncConfigMessage<TData>, TData extends BalmConfig> void register(ResourceLocation resourceLocation,
        ...
        Supplier<TData> copyFactory = SyncConfigMessage.createDeepCopyFactory(() -> BalmConfig.getConfig(dataClass), dataFactory);
        BalmNetworking.registerServerboundPacket(resourceLocation, messageClass, (TMessage message, FriendlyByteBuf buf) -> {
            TData data = message.getData();
            writeSyncedFields(buf, data);
        }, buf -> {
